...
- рассчитывать максимальный поток (распределение ресурсов)
- находить узкие места (bottlenecks)
- находить уязвимости:
	- мосты (critical edges)
	- точки сочленения (critical nodes)
...
### 6) Максимальный поток

`POST /max-flow`

Описание:
- выбирается случайный source (насос)
- выбираются несколько sinks (жилые модули)
- считается максимальный поток

Пример:
```bash
curl -X POST "http://127.0.0.1:8000/max-flow" \
  -H "Content-Type: application/json" \
  -d '{ "graph": { ... } }'
```
...
Описание:
- ищет критические элементы графа

Ответ:
```json
{
  "bridges": [[1, 2]],
  "articulation_points": [3, 7]
}
```
...
Фронтенд реализует:
- визуализацию графа
- цветовую индикацию для:
	- MST
	- загруженные трубы (bottlenecks)
	- мосты
	- точки сочленения
- клик по узлу → удаление
